...
Industries were provided in the IPUMS data extracted from the IPUMS site. This data was merged with the provided 'indnames.csv' file, with the industry codes mapped to the appropriate industry. We took five variables of interest from the IPUMS dataset: Retail (the main industry of comparison), Arts_Ent_Rec_Accom_Food (Arts, Entertainment, Recreation, Accommodation, and Food industry), FINC_Ins_Real_Estate (Finance, Insurance, and Real Estate industry), Construction (Construction industry), and Public_Admin (Public Administration industry). This variable was turned into a factor variable and reorder() was used to get the factors in the correct order so that comparisons in the export_summs table would be made to the Retail industry.
...
The 'covid' variable was created by dividing the time of our data set from April 2020 onward to signify data 'post-COVID' while data from February 2020 and prior signified data 'pre-COVID'. We removed March 2020 from our data set to better divide the two different times. The plot provided below shows a sharp drop in employment rate between February and April 2020. March 2020 was omitted to clearly draw this line. This is also known as the Doughnut hole approach.  
...
The analyses answer the question being asked by taking into account the interaction of the 'covid' variable with all industries, compared to the Retail industry. In the shown factor comparison included below, the Retail industry is omitted in the export_summs table because the factors are all being compared to it. The interpretation of these numbers are as follows:
