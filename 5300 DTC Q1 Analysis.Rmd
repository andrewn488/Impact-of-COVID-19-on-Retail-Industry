...
COVID-19 protective measures variable information was draw from numerious state and local government websites.
...
Seeing the spike in unemployment at the beginning of 2020 in the plot above, we established an initial hypothesis that COVID-19 measures imposed to reduce the spread of the disease were negatively impacting the retail industry's health. Specifically, we looked at stay-at-home orders (SAH):
...
####lm_robust(layoff ~ lck_dwn_st + factor(state))
...
The model did not show a statistically significant relationship between the SAH start dates and unemployment rates. This is not surprising as the plot shows layoffs began to decline soon after the first SAH orders. The IPMUS variable “covid_impact” also reflects that trend (see above).  Starting in April 2020, IPMUS began asking respondents to report if their unemployment was attributable to COVID. That variable also saw a decline mirroring that of unemployment due to layoffs following the first SAH. 

Thus, SAH appears not to have had a negative impact on the retail sector. To understand what may have led to the sudden increase in unemployment - and decay in the retail sector's health - that occurred at the beginning of the year we decided to look for another factor: the rise in COVID-19 cases.
...
The new model showed a negative relationship between layoffs and cases.  We could interpret this as the retail industry's health improving as COVID-19 cases rose.  Whether there is relationship present or not the plot does show a decline in unemployment while cases were still rising.    
