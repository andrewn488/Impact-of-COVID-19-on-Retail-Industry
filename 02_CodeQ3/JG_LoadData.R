...
df <- df %>% 
  ...
  select(year, month, year_month, wtfinl, ageGeneration, age, racialCategories, 
         female, married, employed, educationCategories, covid, time) 

year_covid <- df %>%
  distinct(year_month, year, month, covid)
...
#Plot
emp_female <- ggplot(female_df_z, aes(year_month)) + 
  geom_line(aes(
...
## join data
race_df <- left_join(df, race_rates_monthly, by = c("year", "month"))
# convert to years 
race_df_z <- race_df %>% 
  unite(year_month, c("year", "month"), sep = "-")  %>%
  mutate(year_month = (Date = as.yearmon(year_month))) %>%
